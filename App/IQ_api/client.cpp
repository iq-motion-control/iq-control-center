...
int Client::Reply(const uint8_t* data, uint8_t len, const std::string &entry_descriptor)
{
  ...
  switch(format)
  {
    case 'n':
    {
      ...
      if(!(entry_ptr = dynamic_cast<ClientEntryVoid*>(abstract_entry_ptr)))
        return -1;
  ...
  if(dir == kReply)
  {
    for( const auto& n : client_entry_map ) 
    {
      ...
      if(sub_idn == n.second->sub_idn_)
      {
        ...
        if(n.second->type_idn_ == type_idn &&
        n.second->obj_idn_ == obj_idn)
        {
...
void CreateClientEntry(const uint8_t &obj_idn, const Json::Value &param, ClientEntryAbstract* &entry_ptr)
{
