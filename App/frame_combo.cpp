...
FrameCombo::FrameCombo(QWidget *parent, Client* client, std::pair<std::string, ClientEntryAbstract*> client_entry, FrameVariables* fv) :
  Frame(parent, 1),
  client_(client),
  client_entry_(client_entry)
{
  ...
  SetPushButton(push_button_default_, size_policy, QString("pushButtonDefault"),  QString(":/res/default_icon.png"));
  ...
  SetPushButton(push_button_save_, size_policy, QString("pushButtonSave"), QString(":/res/save.png"));
...
void FrameCombo::SetPushButton(QPushButton *push_button,QSizePolicy size_policy, QString push_button_name, QString icon_file_name )
{
...
void FrameCombo::GetSavedValue()
{
  if(iv.pcon->GetConnectionState() == 1)
  {
    try
    {
      if(!GetEntryReply(*iv.pcon->GetQSerialInterface(), client_, client_entry_.first, 5, 0.05f, saved_value_))
