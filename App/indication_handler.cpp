...
void IndicationHandler::PlayNote(uint16_t frequency, uint16_t duration){
  ...
  buzzer_control_map_["buzzer_control_client"]->Set(*serial_connection_, "volume", 100);
  ...
  while(response > -1){
    GetEntryReply(*serial_connection_, buzzer_control_map_["buzzer_control_client"], "ctrl_mode", 1, 0.005f, response);
  }
}
